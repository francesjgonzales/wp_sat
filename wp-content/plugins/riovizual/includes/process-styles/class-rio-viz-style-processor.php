...
class Rio_Viz_Style_Processor {
	...
	public function __construct() {
		...
		add_action( 'enqueue_block_assets', array( $this, 'rv_font_post' ) );
	...
	public function save_font( $request ) {
		...
		if( ! isset( $params['blocks_font']['block_id'] ) ) {

			if( ! $save_font ) { return 'success'; }

			// get all font family name from updated save font list.
			$updated_save_font_keys = array_keys( $save_font );
			...
			foreach ( $updated_save_font_keys as $save_font_key ) {
				$save_font_post_ids = array_keys( $save_font[ $save_font_key ]['post_id'] );

				if( in_array($post_id, $save_font_post_ids) ) {
					if ( count( $save_font_post_ids ) === 1 ) {
						unset( $save_font[ $save_font_key ] );
					} else {
						unset( $save_font[ $save_font_key ]['post_id'][ $post_id ] );
					}

					update_option( $option_name, $save_font );
				}
			}
		} else {
